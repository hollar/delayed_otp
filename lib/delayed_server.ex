defmodule DelayedServer do
  ...
  """
  def start_link(mod, args, options \\ []),
    do: GenServer.start_link(__MODULE__, {mod, args, options})
  ...
  def handle_call(req, _from, state),
    do: {:reply, GenServer.call(state.pid, req, state.call_timeout), state}

  def handle_cast(req, state) do
    GenServer.cast(state.pid, req)
    {:noreply, state}
  end
  ...
  def terminate(reason, %{pid: pid, shutdown: shutdown, name: name}) do
    ...
    after
      shutdown ->
        Logger.warn(
          "Delayed server #{name} failed to terminate within #{shutdown}, killing it brutally"
        )
